...
export default function CartItem({ item, isInView, ref }) {
  ...
  return (
    <div ref={ref} className="flex bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
      {/* Product Image */}
      ...
      <div className="flex-1 flex flex-col justify-between p-4 min-w-0">
        <div>
          ...
          <div className="flex gap-1.5 mt-1.5 flex-wrap">
            {item.species.map((sp) => (
              <span key={sp} className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600 capitalize">
                {sp}
              </span>
            ))}
          </div>
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <Price price={(item.pokemons_selling?.regular_price * (1 - (item.pokemons_selling?.discount || 0) / 100)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} />
