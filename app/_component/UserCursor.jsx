...
export default function UserCursor({
  ...
  useEffect(() => {
    ...
    const onLeave = () => { setHovering(false); lastSampleRef.current = null; labelTiltTarget.set(0); };
  ...
  return (
    <div ref={containerRef} className="user-cursor-host" style={{ position: "relative", width: "100%", height: "100%", ...style }}>
      ...
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 9999 }}>
        {showLabel && (
          <motion.div
            style={{
              ...
              x: labelTranslateX, y: labelTranslateY,
              rotate: labelRotation, scale: scaleMV,
              background: color, borderRadius: 999,
              padding: `${size * 0.18}px ${size * 0.36}px`,
              boxShadow: "0 4px 12px rgba(0,0,0,0.12), 0 1px 2px rgba(0,0,0,0.08)",
              opacity: visible ? 1 : 0, transformOrigin: "0% 50%",
              transition: "opacity 140ms ease", willChange: "transform, opacity",
              userSelect: "none", pointerEvents: "none",
            }}
          >
            <div style={{ color: textColor, fontSize: Math.max(7, size * 0.43), lineHeight: 1.1, fontWeight: 600, fontFamily: "system-ui, sans-serif", whiteSpace: "nowrap", letterSpacing: 0.1 }}>
              {name}
            </div>
          </motion.div>
        )}
        <motion.div
          style={{
            position: "absolute", top: 0, left: 0,
            x: arrowX, y: arrowY, scale: scaleMV,
            width: size, height: size,
            opacity: visible ? 1 : 0, transformOrigin: "0% 0%",
            transition: "opacity 140ms ease", willChange: "transform, opacity",
            pointerEvents: "none",
          }}
        >
          <svg width={size} height={size} viewBox="0 0 28 28" fill="none" style={{ display: "block", overflow: "visible" }}>
